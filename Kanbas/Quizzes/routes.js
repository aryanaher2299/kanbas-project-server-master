...
function QuizRoutes(app) {
  ...
  const publishQuiz = async (req, res) => {
    ...
    const status = await dao.updateQuiz(req.params.qid, {
  ...
  const submitQuiz = async (req, res) => {
    ...
    for (let i = 0; i < chosenAnswersData.length; i++) {
      const question = await quizQuestionsDao.findQuestionById(
        chosenAnswersData[i]._id
      );
      if (
        JSON.stringify(question.answer) ===
        JSON.stringify(chosenAnswersData[i].chosenAnswer)
      ) {
    ...
    if (username !== "anonymous") {
      quizPreview = await quizPreviewDao.findAttemptWithUsernameAndQuiz(
        username,
        req.params.qid
      );
    }
    if (quizPreview !== null) {
      res.status(206).send("Quiz submitted previously");
      return;
    }
